		...
		if (metadata) {
			...
			for (let i = 0; i < files.length; i++) {
				try {
					...
					let fileId = await uploadFile(req, files[i].note, filename);
					newFileIDs.push({ oldId: files[i].id, newId: fileId });
					metadataHandler.newFile({
						fileId,
						filename,
						title: files[i].title,
						createdDate: files[i].created,
			...
			try {
				...
				res.end(
					JSON.stringify({
						status: 'failed to update metadata',
	...
	*/
	...
	if (
		...
		if (metadata) {
			...
			for (let i = 0; i < files.length; i++) {
				try {
					await updateFile(req, files[i].note, files[i].id);
					...
					console.log(err);
					erroredFileIds.push(files[i].id);
				}
			}
			try {
				await saveMetaDataFile(req, metadataHandler.metadata);
